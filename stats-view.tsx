...
import { Clock3, Gamepad2, Hammer, Loader2, RefreshCw, Unlock, VolumeX } from "lucide-react";
        ...
        <ol>
          {data.moderators.map((m, i) => (
            ...
            >
              ...
              {online[m.steamid] ? null : <LastSeen info={m.lastOnline} />}
              {m.pct != null ? (
...
function LastSeen({ info }: { info: ModRow["lastOnline"] }) {
  ...
  const day = fmt(d) === fmt(new Date()) ? "сегодня" : fmt(d);
  return (
    <p
      className="mt-1 truncate pl-8 text-xs text-subtle"
      title={[info.nickname, info.map].filter(Boolean).join(" · ") || undefined}
    >
      <Clock3 className="mr-1.5 -mt-0.5 inline size-3.5" />
      Последний раз в игре: {day} в {hm} · <span className="text-fg/80">{info.server ?? "—"}</span>
    </p>
  );
}
